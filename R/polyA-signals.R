  ...
  all.pos <- foreach(chr=chrs, .packages="Biostrings",
    ...
    str.pos <- lapply(c('+', '-'), function(s) {
      ...
      if (!is.null(matches) && nrow(matches) > 0) {
        ...
        matches$start.event <- start(e.strand)[matches$space]
        matches$end.event <- end(e.strand)[matches$space]
        if ('count' %in% colnames(meta)) {
          matches$count <- meta$count[matches$space]
        }
        if ('entrez.id' %in% colnames(meta)) {
          matches$entrez.id <- meta$entrez.id[matches$space]
        }
        if ('exon.anno' %in% colnames(meta)) {
          matches$exon.anno <- meta$exon.anno[matches$space]
        }
        if (s == '+') {
