...
identifyValidPaSignals <- function(x, pa.stats, pa.min.from.end=10L,
  ...
  pas <- transform(pa.stats, start=start.signal - 1L, width=6)
  pa.gr <- as(pas, "GRanges")
  pa.gr <- shift(pa.gr, values(pa.gr)$start.event)

  x.gr <- resize(x, width=pa.max.from.end - pa.min.from.end, fix='end')
  is.fwd <- as.logical(strand(x.gr) == '+')
  x.gr <- shift(x.gr, ifelse(is.fwd, -pa.min.from.end, pa.min.from.end))
  ...
  if (count.only) {
    ...
    ans <- mm[, {
      ...
      first <- signal[rank[1L]]
      second <- default
      third <- default
      fourth <- default
      if (N > 1L) {
        second <- signal[rank[2L]]
      }
      if (N > 2L) {
        third <- signal[rank[3L]]
      }
      if (N > 3L) {
        fourth <- signal[rank[4L]]
      }
